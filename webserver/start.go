...
func StartWebServer(extsTmpDir string, browserTmpDir string, isChrome bool, forceClose chan struct{}) error {
	...
	r.GET("/proxy/:appid/:url", func(c *gin.Context) { proxyHandeler(c, "GET", newReq) })
	r.POST("/proxy/:appid/:url", func(c *gin.Context) { proxyHandeler(c, "POST", newReq) })
	r.OPTIONS("/proxy/:appid/:url", func(c *gin.Context) { proxyHandeler(c, "POST", newReq) })

	r.GET("/lastRequests", lastRequests)
	r.GET("/extensionsInfo", extensionsInfo)
	r.GET("/requestInfo/:id", requestInfo)
	r.GET("/extLogo/:extID", func(c *gin.Context) { extLogo(c, extsTmpDir) })
